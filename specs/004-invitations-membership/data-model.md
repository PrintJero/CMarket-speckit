...
- Accepting an invitation MUST, atomically: verify the accepting `Account`'s email matches `Invitation.email` via case-insensitive exact match (FR-002, reusing `emailsMatch()`), verify `Account.emailVerifiedAt` is set (FR-003, reusing `assertEmailVerified()`), mark the `Invitation` consumed, and create the `Membership` row — all in one `prisma.$transaction` (research.md #5's sibling pattern; mirrors `createCommunity()`'s atomic community+membership insert). If any check fails, no write occurs.
...
2. **`acceptInvitation()`**: marking the `Invitation` consumed and creating the `Membership` row (FR-005) — either both happen or neither does, preventing a consumed-but-membership-less or membership-but-still-pending inconsistent state.
3. **`revokeMembership()`**: counting remaining administrators and deleting the target `Membership` row (FR-009) — the count-then-delete MUST happen inside one transaction to close the concurrent-revoke race described in research.md #5.
