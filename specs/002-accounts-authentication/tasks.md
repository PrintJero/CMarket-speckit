...
## Delta: FR-025 federated sign-in session cookie unification bug-fix (2026-07-17)

**Scope**: In the deployed app, Google sign-in completed (Auth.js returned a successful callback redirect, no error param, and created a `Session` row) but `getCurrentAccount()` returned `null` on the very next request, rendering the signed-out view. Root cause: `getCurrentAccount()` reads a cookie named `cmarket_session` (the name the custom email/password routes set), while Auth.js, left unconfigured in `authConfig.ts`, sets its own default cookie name (`next-auth.session-token`). Token hashing and `Session` row creation were already consistent between the two paths (the custom adapter's `createSession()` hashes before storing, the lookup path hashes the same way before querying) — the entire defect was the cookie name mismatch. Every existing Google integration test calls the Auth.js adapter's methods directly and asserts on database rows only, by design (driving Google's real consent screen isn't feasible in tests), so none of them ever touched a cookie or `getCurrentAccount()` — the exact seam the bug lived in. Fix: configure Auth.js's `cookies.sessionToken.name`/`options` to match the app's existing `SESSION_COOKIE_NAME`/`sessionCookieOptions` exactly, rather than teaching `getCurrentAccount()` to check two cookie names. This delta covers spec → tests → implementation for this fix only; no other behavior in this feature was touched.

- [X] T078 Amend spec.md: add Clarifications session (bug-fix amendment) describing the two-cookie-name defect and the decision to unify on one name rather than reading two; add new FR-025 (federated sign-in MUST resolve via the same single session cookie as email/password, sign-out MUST clear it and revoke the `Session` row regardless of which method created it); add new SC-013 (verified by a test that sets the real Auth.js-configured cookie and loads a page, not just database assertions).
- [X] T079 Add new unit test `tests/unit/test_auth_session_cookie_unification.ts`: asserts `authOptions.cookies?.sessionToken?.name === SESSION_COOKIE_NAME` — a direct regression guard against this exact drift recurring. **Confirmed RED** (`authOptions.cookies` was `undefined` pre-fix).
- [X] T080 Add new integration test `tests/integration/test_google_session_cookie.spec.ts`, test 1: creates an account + links it via the real Auth.js adapter (mirroring the other Google tests' own precedent for not driving a live consent screen), calls `adapter.createSession()` directly to get a raw token, sets it as a real browser cookie under whatever name Auth.js is actually configured to write (`authOptions.cookies?.sessionToken?.name`, falling back to NextAuth's own unconfigured default for this http/non-secure test environment), loads `/`, and asserts the signed-in view renders for that account. **Confirmed RED** (rendered the signed-out "Welcome" view instead).
- [X] T081 Add test 2 to the same file: after establishing sign-in the same way, clicks "Sign out" and asserts (a) the signed-out view renders, (b) the browser no longer carries the session cookie, and (c) the underlying `Session` row is gone — verifying sign-out revocation (FR-008) also works correctly for a Google-established session, not only an email/password one. **Confirmed RED** for the same underlying reason (the cookie sign-out reads was never the one Google's callback would have set, so this test could not even reach a signed-in state to sign out of).
- [X] T082 Implement: add `cookies: { sessionToken: { name: SESSION_COOKIE_NAME, options: sessionCookieOptions } }` to `authOptions` in `src/lib/auth/authConfig.ts`, importing both from `@/lib/auth/sessionCookie`. No change to `getCurrentAccount()`, the custom adapter, or the sign-out route — only Auth.js's own cookie configuration.
- [X] T083 Re-run T079–T081 — **confirmed GREEN** — then the full suite: `tsc --noEmit`, `eslint .`, `vitest run tests/unit tests/contract`, `playwright test` (all specs) — confirming no regressions in any prior delta's tests, including the four Google adapter-level tests (unaffected, since they never touch a cookie).
