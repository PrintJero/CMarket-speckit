...
// the ONLY place the mobile app talks to the backend — no invitation/membership
// business logic is reimplemented here, just request/response shaping.
// Mirrors web/src/services/apiClient.ts so both clients consume the
// identical contract (Constitution Principle V: Web/Mobile Parity).
...
function getBaseUrl(): string {
  return process.env.EXPO_PUBLIC_API_BASE_URL ?? 'http://localhost:4000/api/v1';
}
...
export function acceptInvitation(invitationId: string): Promise<AcceptInvitationResponse> {
  return request<AcceptInvitationResponse>(`/invitations/${invitationId}/accept`, {
    method: 'POST',
  });
}
...
export function cancelInvitation(communityId: string, invitationId: string): Promise<void> {
  return request<void>(`/communities/${communityId}/invitations/${invitationId}`, {
    method: 'DELETE',
  });
}
