...
test("repeating sign-up for the same email returns an identical response", async ({
  ...
  const first = await request.post("/api/auth/sign-up", {
    data: { displayName: "Enum Tester", email, password },
  });
  expect(first.status()).toBe(202);
  const firstBody = await first.json();

  const second = await request.post("/api/auth/sign-up", {
    data: { displayName: "Enum Tester", email, password: "a-different-valid-password" },
  });
